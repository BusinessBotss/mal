...
import { getSunsetSpots, getCategoryConfig } from "../../../content/utils";
import { PageIntro } from "@/components/PageIntro";
import { FilterableListingPage } from "@/components/FilterableListingPage";

const cat = getCategoryConfig("sunset-spots");
...
export default function SunsetSpotsPage() {
  const items = getSunsetSpots();
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16">
      <PageIntro eyebrow={cat.eyebrow} title={cat.label} subtitle={cat.description} />
      <FilterableListingPage items={items} basePath="/sunset-spots" emptyText={cat.emptyText} />
    </div>
  );
}
